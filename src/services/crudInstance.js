...
class Crud {
 ...
 async _requestWithRefresh(method, url, data) {
  try {
    ...
    // تحقق إذا انتهت صلاحية الـ access token
    if (res.status === 401 && url !== API.AUTH.REFRESH) {
      console.log(`[Crud] Access token expired at ${new Date().toLocaleTimeString()}`);

      const refresh_token = getCookie('refresh_token');
      if (!refresh_token) {
        console.warn('[Crud] Refresh token missing');
      ...
      console.log('[Crud] New access token received at', new Date().toLocaleTimeString(), newToken);
